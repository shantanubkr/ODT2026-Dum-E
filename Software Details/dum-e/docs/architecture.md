...
| `src/modules/` | **`state_machine.py`** — `States` and transitions. **`command_parser.py`** — text → `{type, command, args}`. **`intent_parser.py`** — extends parser with **sentiment** (`GREET` / `BYE` / `HAPPY` / `SAD` / `NEUTRAL`) for expression behaviors. **`motion_controller.py`** — five PWM servos, named poses (`home`, `ready`, `down`). **`behavior_engine.py`** — idle / greeting / thinking + `express_*` behaviors. **`safety_manager.py`** — emergency stop + sensor fault latches; `can_move()` gates motion in the router. |
| `src/drivers/` | **`stepper.py`** — `Servo` (MicroPython `PWM` hobby servo; filename is legacy). **`pir.py`** — `Button` (debounced + held) for optional panel wiring. No ToF/ultrasonic drivers in-tree yet. |
